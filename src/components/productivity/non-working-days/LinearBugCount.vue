<template>
  <div
    v-if="
      -20 < differencePercentMetrics(current_linear_bug_created_count, previous_linear_bug_created_count) &&
      differencePercentMetrics(current_linear_bug_created_count, previous_linear_bug_created_count) < 20
    "
  >
    <p class="text-black whitespace-nowrap items-center flex">
      {{ current_linear_bug_created_count || 0 }}
      <span
        v-if="differenceEventCount(current_linear_bug_created_count, previous_linear_bug_created_count) > 0"
        class="text-black items-center flex"
        ><arrow-up-icon class="w-4 h-4 text-black-600"></arrow-up-icon
        >{{ differenceEventCount(current_linear_bug_created_count, previous_linear_bug_created_count) || 0 }}</span
      >
      <span
        v-if="differenceEventCount(current_linear_bug_created_count, previous_linear_bug_created_count) < 0"
        class="text-black items-center flex"
    ...
    <p class="text-gray-400 text-sm whitespace-nowrap">пред. неделя: {{ previous_linear_bug_created_count || 0 }}</p>
  </div>

  <div
    v-else-if="
      -40 < differencePercentMetrics(current_linear_bug_created_count, previous_linear_bug_created_count) &&
      differencePercentMetrics(current_linear_bug_created_count, previous_linear_bug_created_count) < -20
    "
  >
  ...
  <div
    v-else-if="differencePercentMetrics(current_linear_bug_created_count, previous_linear_bug_created_count) > 20"
  >
    <p class="whitespace-nowrap items-center flex">
      {{ current_linear_issues_created_count || 0 }}
      <span
...
<script>
  ...
  export default {
    ...
    props: {
      current_linear_bug_created_count: Number,
      previous_linear_bug_created_count: Number,
    },
    setup(props) {
      return {
