...
<script>
  ...
  import { allProductivityMonthly, allProductivityWeekly } from '~/services/core/productivity.api';

  export default {
    name: 'EmployeeDataMetric',
    components: {
      AtPagination,
    ...
    setup() {
      const current_linear_issues_created_count = ref(Number);
      ...
      const getEmployeesMonthlyMetric = () => {
        loading.value = true;
        allProductivityMonthly(query.value)
          .then(response => {
      ...
      const getEmployeesWeeklyMetric = () => {
        loading.value = true;
        allProductivityWeekly(query.value)
          .then(response => {
            indexMetric.value = response?.data.data;
            indexPagination.value = response?.data;
          })
      ...
      getEmployeesMonthlyMetric();

      const changePeriodMetrics = value => {
        period.value = value;
        if (period.value == 'weekly') {
          getEmployeesWeeklyMetric(query.value);
        } else {
          getEmployeesMonthlyMetric(query.value);
        }
      };
