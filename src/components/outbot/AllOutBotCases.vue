<template>
      ...
      <div v-else>
        ...
        <at-table :head="messagesTableHead" :elements="outBotData">
          <template #element="{ element, index }">
            ...
            >
              <at-badge :color="outBotCasesColor[element?.type]" rounded> {{ outBotCases[element?.type] }}</at-badge>
            </at-table-item>
            <at-table-item
              :to="{
                name: 'colleagues.detail',
            ...
            >
              <p class="text-gray-500 whitespace-nowrap">
                {{ formatDateByDayMonthYear(element?.date) || '-' }}
              </p>
              <p class="text-gray-500 whitespace-nowrap">{{ $filters.readableTime(element?.date) || '-' }}</p>
            </at-table-item>
          </template>
          <template #footer>
            <div class="flex justify-between items-center">
              <p class="justify-start text-gray-400">
                Показано от <span>{{ indexPagination?.from || '0' }}</span> до
                <span> {{ indexPagination?.to || '0' }}</span> из
                <span>{{ indexPagination?.total || '0' }}</span>
              </p>
            </div>
  ...
  <div class="mt-2 flex justify-end">
    <at-pagination
      v-if="indexPagination?.last_page > 1"
      class="py-4 md:col-span-1 justify-end"
      v-model="query.page"
      :total="indexPagination?.last_page"
    ></at-pagination>
  </div>
</template>
...
<script>
  ...
  import { getOutBotIndex } from '~/services/core/outbotMessages';
  ...
  export default {
    ...
    setup() {
      ...
      const outBotCasesColor = {
        'day_offs': 'purple',
        'self_reason_cases': 'green',
        'late_cases': 'red',
        'remote_work_cases': 'yellow',
      };

      const outBotCases = {
        'day_offs': 'Отпроситься',
        'self_reason_cases': 'Предупредить',
        'late_cases': 'Опаздываю',
        'remote_work_cases': 'Работаю удалённо',
      };

      const allColleagueOutbotMessage = () => {
      ...
      watch(
        ...
        debounce(() => {
          ...
          allColleagueOutbotMessage(query.value);
        }, 500),
        { immediate: true }
      );
