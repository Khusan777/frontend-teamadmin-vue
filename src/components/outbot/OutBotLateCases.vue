      ...
      <div v-else>
        ...
        <at-table :head="messagesTableHead" :elements="outBotData">
          <template #element="{ element, index }">
            ...
            >
              <at-badge color="red" rounded>Опаздывает</at-badge>
            </at-table-item>
            <at-table-item
              :to="{
                name: 'colleagues.detail',
            ...
            >
              ...
              <p class="text-gray-500 whitespace-nowrap">{{ element.minute || '-' }} минут</p>
            </at-table-item>
          </template>
          <template #footer>
...
<script>
  ...
  import { getLateCasesMessages } from '~/services/core/outbotMessages';

  export default {
    name: 'OutBotLateCases',
    components: {
      AtBadge,
      AtPagination,
      AtTable,
    ...
    setup() {
      ...
      const colleagueLateCasesMessage = () => {
      ...
      watch(
        ...
        debounce(() => {
          ...
          colleagueLateCasesMessage(query.value);
        }, 500),
        { immediate: true }
      );
