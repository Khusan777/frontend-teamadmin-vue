      ...
      <div v-else>
        ...
        <at-table :head="messagesTableHead" :elements="outBotData">
          <template #element="{ element, index }">
            ...
            >
              <at-badge color="purple" rounded>Отпрашивание</at-badge>
            </at-table-item>
            <at-table-item
              :to="{
                name: 'colleagues.detail',
            ...
            >
              ...
              <p>
                ...
                }}<span class="text-gray-400 ml-2">{{ element?.duration_minute }} мин</span>
              </p>
            </at-table-item>
          </template>
          <template #footer>
...
<script>
  ...
  import { formatDateByDayMonthYear, parseDate, readableDateTime } from '~/utils/filters/dateFilters';
  import { debounce } from '~/plugins/aliftech-ui/utils';
  import { useRoute } from 'vue-router';
  import useRouterReplace from '~/utils/hooks/useRouterReplace';
  import { ref, watch } from 'vue';
  import { $toast } from '~/main';
  import { getDayOffsMessages } from '~/services/core/outbotMessages';
  ...
  export default {
    ...
    setup() {
      ...
      const colleagueOutbotDayOffMessage = () => {
      ...
      watch(
        ...
        debounce(() => {
          ...
          colleagueOutbotDayOffMessage(query.value);
        }, 500),
        { immediate: true }
      );
