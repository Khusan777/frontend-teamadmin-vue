<template>
  ...
  <div v-else v-for="activityList in activities" :key="activityList.id">
    <div v-if="activities.length && activityList.employees.length" class="mt-4 pt-6 border-t">
      <div>
        <h1 class="text-2xl font-bold mb-4">Отдел - {{ activityList?.department_name }}</h1>
        <p class="text-gray-400 text-xl mb-4">
          Команда - <span class="text-2xl text-black font-bold">{{ activityList?.name }}</span>
        </p>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        <div
